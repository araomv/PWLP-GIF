function [f0,VUVDecisions,SRHVal] = SRH_PitchTracking(wave,Fs,F0min,F0max)
...
for Iter=1:Niter   
    ...
    [f0,SRHVal] = SRH_EstimatePitch(res',Fs,F0min,F0max);
    ...
    if length(posiTmp)>1
        ...
        F0min=max(round(0.5*F0meanEst),50);
        F0max=min(round(2*F0meanEst),500);
...
function [f0,SRHVal] = SRH_EstimatePitch(sig,Fs,F0min,F0max)
